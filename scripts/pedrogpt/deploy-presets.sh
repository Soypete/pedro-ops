...
if [[ "$USE_TAILDROP" == "true" ]]; then
  ...
  ssh "$REMOTE_HOST" "sudo mkdir -p $REMOTE_PRESETS_DIR"
  scp "$PRESETS_DIR"/*.ini "$REMOTE_HOST:/tmp/"
  ssh "$REMOTE_HOST" "sudo mv /tmp/*.ini $REMOTE_PRESETS_DIR/ && sudo chmod 644 $REMOTE_PRESETS_DIR/*.ini"
...
EOF
    ...
    if ssh "$REMOTE_HOST" "sudo systemctl is-active --quiet $SERVICE_NAME"; then
      ...
      ssh "$REMOTE_HOST" "sudo journalctl -u $SERVICE_NAME -n 30"
      exit 1
    fi
  else
